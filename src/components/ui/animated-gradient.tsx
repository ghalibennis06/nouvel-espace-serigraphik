...
interface PresetParams {
  ...
  softness: number; offset: number;
  shape: PatternShape; shapeSize: number;
}
...
export default function AnimatedGradient({
  ...
  const params = useMemo((): PresetParams => {
    if (config.preset === "custom") {
      return {
        ...
        softness: config.softness ?? 100, offset: config.offset ?? 0,
        shape: config.shape ?? "Checks", shapeSize: config.shapeSize ?? 10,
      };
    }
  ...
  useEffect(() => {
    ...
    const uniforms = {
      u_time: u("u_time"), u_resolution: u("u_resolution"), u_pixelRatio: u("u_pixelRatio"),
      u_scale: u("u_scale"), u_rotation: u("u_rotation"),
      u_color1: u("u_color1"), u_color2: u("u_color2"), u_color3: u("u_color3"),
      u_proportion: u("u_proportion"), u_softness: u("u_softness"),
      u_shape: u("u_shape"), u_shapeScale: u("u_shapeScale"),
      u_distortion: u("u_distortion"), u_swirl: u("u_swirl"),
      u_swirlIterations: u("u_swirlIterations"),
    };
...
function hexToRgba(hex: string): [number, number, number, number] {
  let r = 0, g = 0, b = 0, a = 1;
  if (hex.startsWith("rgba(")) { const p = hex.slice(5, -1).split(","); return [parseInt(p[0])/255, parseInt(p[1])/255, parseInt(p[2])/255, parseFloat(p[3])]; }
  if (hex.startsWith("rgb(")) { const p = hex.slice(4, -1).split(","); return [parseInt(p[0])/255, parseInt(p[1])/255, parseInt(p[2])/255, 1]; }
  if (hex.startsWith("#")) {
    const c = hex.slice(1);
    if (c.length === 3) { r = parseInt(c[0]+c[0],16)/255; g = parseInt(c[1]+c[1],16)/255; b = parseInt(c[2]+c[2],16)/255; }
    else if (c.length >= 6) { r = parseInt(c.slice(0,2),16)/255; g = parseInt(c.slice(2,4),16)/255; b = parseInt(c.slice(4,6),16)/255; if (c.length === 8) a = parseInt(c.slice(6,8),16)/255; }
  }
  return [r, g, b, a];
}
