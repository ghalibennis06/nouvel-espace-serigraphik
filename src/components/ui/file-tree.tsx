    ...
    <div className="select-none">
      ...
      >
        ...
        <div className={cn("flex items-center justify-center w-4 h-4 transition-transform duration-200 ease-out", isFolder && isOpen && "rotate-90")}>
          {isFolder ? (
            <svg width="6" height="8" viewBox="0 0 6 8" fill="none" className={cn("transition-colors duration-200", isHovered ? "text-primary" : "text-muted-foreground")}>
        ...
        <div className={cn(
          ...
          isFolder
            ...
            : isHovered ? cn(fileIcon.color, "scale-110") : cn(fileIcon.color, "opacity-70"),
        )}>
          {isFolder ? (
...
export function FileTree({ data, className }: FileTreeProps) {
  return (
    <div className={cn("bg-file-tree-bg rounded-lg border border-border/50 p-3 font-mono", className)}>
