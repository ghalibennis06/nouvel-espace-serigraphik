...
export default function ClosingDecisionStation({ locale }: { locale: string }) {
  return (
    <section style={{ background: '#1f120d', padding: '92px 5%', borderTop: '1px solid rgba(255,255,255,0.06)', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: 1240, margin: '0 auto', position: 'relative', zIndex: 2 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1.02fr 0.98fr', gap: 24, alignItems: 'start' }} className="grid lg:grid-cols-2 gap-6">
          <div style={{ paddingRight: 10 }}>
            ...
            <h2 style={{ fontSize: 'clamp(34px,4.4vw,58px)', fontWeight: 950, color: '#fff4ed', letterSpacing: '-0.04em', lineHeight: 0.98, marginBottom: 16 }}>
            ...
            {DECISION_LANES.map((lane) => {
              const content = (
                <div style={{ borderRadius: 24, border: '1px solid rgba(255,255,255,0.10)', background: lane.tone, padding: 22, boxShadow: '0 20px 60px rgba(0,0,0,0.14)', transition: 'transform 0.25s ease, border-color 0.25s ease' }}>
                  ...
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '12px 16px', borderRadius: 14, background: lane.external ? 'rgba(37,211,102,0.16)' : 'rgba(255,255,255,0.08)', border: lane.external ? '1px solid rgba(37,211,102,0.30)' : '1px solid rgba(255,255,255,0.12)', color: '#fff', fontSize: 13, fontWeight: 800 }}>
                    {lane.cta} →
                  </div>
                </div>
              )
