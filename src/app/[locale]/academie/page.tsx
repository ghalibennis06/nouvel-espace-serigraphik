...
const LEVEL_STYLES: Record<string, { bg: string; color: string }> = {
  'Débutant':      { bg: 'rgba(15,144,128,0.15)',  color: '#0F9080' },
  'Intermédiaire': { bg: 'rgba(59,130,246,0.15)',   color: '#60A5FA' },
  'Avancé':        { bg: 'rgba(139,92,246,0.15)',   color: '#A78BFA' },
  'Business':      { bg: 'rgba(200,137,31,0.15)',   color: '#C8891F' },
}
...
export default function AcademiePage({ params }: { params: { locale: string } }) {
  ...
  return (
    <div style={{ background: '#0C0A08', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#1A1612', borderBottom: '1px solid rgba(200,137,31,0.12)', padding: '72px 6% 56px' }}>
        <div style={{ maxWidth: 860, margin: '0 auto', textAlign: 'center' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(200,137,31,0.1)', border: '1px solid rgba(200,137,31,0.25)', borderRadius: 20, padding: '6px 16px', marginBottom: 24 }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: '#C8891F', letterSpacing: '0.08em' }}>📖 ACADÉMIE NES — CONTENU 100% GRATUIT</span>
          </div>
          <h1 style={{ fontFamily: '"Cormorant Garamond",Georgia,serif', fontSize: 'clamp(36px,5vw,58px)', fontWeight: 700, color: '#F5EDD8', lineHeight: 1.1, marginBottom: 20 }}>
            Maîtrisez l&apos;impression<br />
            <span style={{ color: '#C8891F' }}>textile au Maroc.</span>
          </h1>
          <p style={{ fontSize: 16, color: '#B8AA94', lineHeight: 1.75, maxWidth: 560, margin: '0 auto 40px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: '12px 40px' }}>
            {STATS.map(s => (
              <div key={s.label} style={{ textAlign: 'center' }}>
                <div style={{ fontFamily: '"Cormorant Garamond",Georgia,serif', fontSize: 28, fontWeight: 700, color: '#C8891F', lineHeight: 1 }}>{s.n}</div>
                <div style={{ fontSize: 11, color: '#B8AA94', marginTop: 2 }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 56 }}>
          {GUIDES.map(section => (
            <div key={section.category}>
              {/* Section header */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 24, paddingBottom: 20, borderBottom: '1px solid rgba(245,237,216,0.06)' }}>
                ...
                <div>
                  <h2 style={{ fontFamily: '"Cormorant Garamond",Georgia,serif', fontSize: 24, fontWeight: 700, color: '#F5EDD8', lineHeight: 1.1 }}>{section.category}</h2>
                  <p style={{ fontSize: 12, color: '#B8AA94', marginTop: 2 }}>{section.articles.length} guides</p>
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 14 }}>
                {section.articles.map(article => (
                  <Link
                    ...
                    style={{ display: 'flex', flexDirection: 'column', background: '#1A1612', border: `1px solid rgba(245,237,216,0.07)`, borderRadius: 10, padding: 22, textDecoration: 'none', transition: 'border-color .2s, transform .2s' }}
                    className="art-item"
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                      <span style={{
                        fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 4,
                        background: LEVEL_STYLES[article.level]?.bg ?? 'rgba(245,237,216,0.08)',
                        color: LEVEL_STYLES[article.level]?.color ?? '#B8AA94',
                      }}>
                        {article.level}
                      </span>
                      <span style={{ fontSize: 11, color: '#B8AA94' }}>⏱ {article.read}</span>
                    </div>
                    <h3 style={{ fontSize: 14, fontWeight: 700, color: '#F5EDD8', lineHeight: 1.4, marginBottom: 8, flex: 1 }}>
                      {article.title}
                    </h3>
                    <p style={{ fontSize: 12, color: '#B8AA94', lineHeight: 1.65, marginBottom: 16, display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                      {article.desc}
                    </p>
      ...
      {/* ── BOTTOM CTA ─────────────────────────────────────────────────────── */}
      <div style={{ background: '#1A1612', borderTop: '1px solid rgba(245,237,216,0.06)', padding: '72px 6%' }}>
        <div style={{ maxWidth: 680, margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 15, color: '#B8AA94', lineHeight: 1.7, marginBottom: 36 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'center' }}>
            <a
              ...
              style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '14px 28px', background: '#25D366', color: '#fff', borderRadius: 8, fontSize: 15, fontWeight: 700, textDecoration: 'none' }}
            >
            ...
            <Link
              href={`/${locale}/kits`}
              style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '14px 28px', background: '#C8891F', color: '#0C0A08', borderRadius: 8, fontSize: 15, fontWeight: 700, textDecoration: 'none' }}
            >
              Voir les Kits Démarrage →
            </Link>
          </div>
          <p style={{ fontSize: 12, color: '#B8AA94', marginTop: 16 }}>
