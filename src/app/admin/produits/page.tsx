...
export default async function ProduitsPage() {
  const { data: products } = await supabase
    .from('nes_products')
    .select('id, product_slug, name_fr, price, stock_status, active, featured, category_id, created_at')
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28 }}>
        <div>
          ...
          <p style={{ fontSize: 14, color: 'var(--text2)' }}>
            {products?.length ?? 0} produit{(products?.length ?? 0) !== 1 ? 's' : ''} en base
          </p>
        </div>
      ...
      {(!products || products.length === 0) ? (
        ...
        <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {products.map((p, i) => (
                <tr key={p.id} style={{ borderTop: '1px solid var(--border)', background: i % 2 === 0 ? 'transparent' : 'var(--surface)' }}>
                  <td style={{ padding: '12px 16px' }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>{p.name_fr}</div>
                    <div style={{ fontSize: 11, color: 'var(--text2)' }}>{p.product_slug}</div>
                  </td>
