    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, minmax(0, 1fr))', gap: 12, marginBottom: 24 }} className="grid md:grid-cols-7 gap-3">
        {[
          ...
          ['Sans owner', (leads ?? []).filter((lead) => !lead.assignee).length, '#7c3aed'],
          ['Sans suivi planifié', (leads ?? []).filter((lead) => !lead.next_follow_up_at && !['won', 'closed', 'lost', 'spam'].includes(lead.status)).length, '#f26316'],
