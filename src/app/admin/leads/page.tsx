...
export default async function LeadsPage() {
  const { data: leads } = await supabase
    .from('nes_leads')
    .select('id, name, company, phone, email, message, status, notes, created_at, source, segment, request_type, priority, assignee, next_follow_up_at, city, category_interest, product_interest, budget_range')
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 12, marginBottom: 24 }} className="grid md:grid-cols-4 gap-3">
        {[
